...
namespace UnityEngine.AppylarSdkWrapper.Platform.Android {
  internal class AndroidPlatform: INativePlatform, AppylarBannerListener {
    private
    const string ADS_BASE_CLASS = "com.appylar.android.sdk.unity.UnityManager";
    private
    const string ADS_POSITION_CLASS = "com.appylar.android.sdk.enums.Position";
    private
    const string ADS_BANNER_MANAGER_CLASS = "com.appylar.android.sdk.bannerview.SingleBannerManager";
    private
    const string ADS_INTERSTITIAL_CLASS = "com.appylar.android.sdk.interstitial.Interstitial";
    private
    ...
    public void SetupPlatform(IPlatform platform) {
      ...
      singleBannerManagerNative = new AndroidJavaObject(ADS_BANNER_MANAGER_CLASS, m_CurrentActivity, new AndroidAppylarBannerListener(this));
    }

    public void Initialize(string appKey, AdType[] adTypes, bool testMode, AppylarInitializationListener initializationListener) {
      ...
      appylarNative.CallStatic("setEventListener", new AndroidAppylarInitializationListener(initializationListener));
    ...
    public void ShowBanner(BannerPosition position, string placementId, AppylarBannerListener bannerListener) {
      ...
      if (position == BannerPosition.TOP) {
        AndroidJavaObject topEnumValue = positionEnumNative.CallStatic < AndroidJavaObject > ("valueOf", "TOP");
        singleBannerManagerNative.Call("showBanner", topEnumValue, placementId);
      }

      if (position == BannerPosition.BOTTOM) {
        AndroidJavaObject bottomEnumValue = positionEnumNative.CallStatic < AndroidJavaObject > ("valueOf", "BOTTOM");
        singleBannerManagerNative.Call("showBanner", bottomEnumValue, placementId);
      }
    }

    public void ShowInterstitial(string placementId, AppylarInterstitialListener interstitialListener) {
      appylarInterstitial.CallStatic("setEventListener", new AndroidAppylarInterstitialListener(interstitialListener));
